...
export const validateRequest = async (
  ...
  const prompt = [
    ...
    '- "advice": просит СОВЕТ/ПОДБОР — «что мне пойдёт», «подбери образ», «какой стиль мне подходит»,',
    "  «что купить», «посоветуй», «какие цвета мне идут», вопросы про свою внешность/цветотип.",
    '- "tryon": просит ПРИМЕРИТЬ/ПОКАЗАТЬ на себе конкретный стиль или вещи — «одень меня в…»,',
    "  «примерь», «покажи меня в деловом костюме», описание конкретного образа без вопроса о совете.",
    '- Сомневаешься между ними — выбирай "tryon".',
    "",
    "### Текст пользователя",
...
export const adviseOutfit = async (
  ...
  const prompt = [
    ...
    '  "items": [',
    '    {"title": "короткое название вещи для показа пользователю", "query": "поисковый запрос для маркетплейса: вещь + цвет + фасон + пол"}',
    "  ],",
